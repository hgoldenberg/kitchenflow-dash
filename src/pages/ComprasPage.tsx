    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Generar órdenes de compra desde alertas de stock, recibir mercadería y registrar facturas.
        </p>
      </div>
                ...
                <div key={ing.id} className="flex items-center justify-between text-sm bg-card rounded-lg p-3 border">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Stock: {ing.stockActual} {ing.unidad} · Sugerido: {cantidadSugerida} {ing.unidad} · Proveedor: {proveedores[0].nombre}
                    </p>
                  </div>
                  <Button
                    size="sm"
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {ordenesCompra.map((orden) => {
                    ...
                    const tieneFactura = false; // simplified
                    return (
                      <tr key={orden.id} className="border-t">
                        ...
                        <td className="px-4 py-3 text-center space-x-1">
                          {orden.estado === "emitida" && (
                            <Button size="sm" variant="outline" className="gap-1 text-xs" onClick={() => recibirMercaderia(orden.id)}>
                              <PackageCheck className="h-3 w-3" />
                              Recibir
                            </Button>
                          )}
                          {orden.estado === "recibida" && (
                            <Button size="sm" variant="outline" className="gap-1 text-xs" onClick={() => registrarFactura(orden.id)}>
